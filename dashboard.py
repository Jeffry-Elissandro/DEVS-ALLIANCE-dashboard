...
# Código JavaScript para crear partículas con posición aleatoria y retrasos
html = """
<div id="embedim--particles"></div>
<script>
const container = document.getElementById('embedim--particles');
const numParticles = 20; // Número de partículas

for (let i = 0; i < numParticles; i++) {
  const particle = document.createElement('div');
  particle.className = 'particle';

  // Posición inicial aleatoria en el eje X (porcentaje)
  const startX = Math.random() * 100; // porcentaje
  particle.style.left = startX + '%';

  // Añadir retraso para que no todas comiencen al mismo tiempo
...
# Mostrar los estilos y el HTML en la página
st.markdown(css + html, unsafe_allow_html=True)

# Sección que muestra el fragmento de código proporcionado
codigo_mostrado = """
...
st.markdown("### Fragmento de código utilizado:")
st.markdown(f"```js\n{codigo_mostrado}\n```")
