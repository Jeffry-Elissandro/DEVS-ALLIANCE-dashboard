...
# Generar posiciones aleatorias y retrasos
positions = [random.randint(0, 100) for _ in range(num_particles)]
delays = [random.uniform(0, 10) for _ in range(num_particles)]  # retraso aleatorio para más dispersión
...
.particle {
  position: absolute;
  width: 6px; /* tamaño grande */
  height: 6px; /* tamaño grande */
  background: white;
  border-radius: 50%;
  opacity: 0.8;
  box-shadow: 0 0 10px 3px white; /* destello */
  animation: floatUp 20s linear forwards;
}
</style>
"""

# Crear HTML con partículas en posiciones aleatorias y retrasos diferentes
html_particles = ""
for i in range(num_particles):
    left_percent = positions[i]
    delay = delays[i]
    html_particles += f'<div class="x" style="left: {left_percent}%; animation-delay: {delay}s;"><div class="particle"></div></div>'

# Unir todo en HTML completo
html = f"""
